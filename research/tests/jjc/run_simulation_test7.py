"""
CRS × UserSim 오케스트레이션 스크립트 (test7).

test5 대비 변경:
- graph_test5 사용: use_hyde 파라미터 지원
- query_transform에 "hyde" 추가: HyDE RAG vs 기본(none) 비교

실행:
    cd /home/jjeong3150/work/peekabook/backend
    python ../research/tests/jjc/run_simulation_test7.py

Sweep 실행:
    cd /home/jjeong3150/work/peekabook/backend
    python ../research/tests/jjc/run_simulation_test7.py --sweep
"""
import asyncio
import copy
import gc
...
SWEEP_CONFIG = {
    ...
    "parameters": {
        ...
        "query_transform":  {"values": ["rewrite"]},
    ...
    while True:
        ...
        if isinstance(message, dict) and message.get("__done__"):
            ...
            result_collector.append({
                ...
                "hypothetical_doc": crs_result.get("hypothetical_doc", ""),
...
async def main():
    ...
    if query_transform == "hyde":
        import app.rag.query_transform_hyde as qt_hyde
        qt_hyde.QDRANT_COLLECTION_NAME = os.getenv("QDRANT_COLLECTION_NAME", "")
        app_ = create_app(chroma_db_path=chroma_db_path, use_genre_filter=use_genre_filter,
                          use_hyde=True)
    else:
        ...
        app_ = create_app(chroma_db_path=chroma_db_path, use_genre_filter=use_genre_filter,
                          use_hyde=False, rag_module=qt_v5)

    session_id = uuid.uuid4().hex[:8]
    thread_id  = f"single_test7_{session_id}"

    results = []
    with get_openai_callback() as cb:
        await run_session(app_, persona, results, thread_id=thread_id, session_id=session_id)
...
def run():
    ...
    if cfg.query_transform == "hyde":
        import app.rag.query_transform_hyde as qt_hyde
        qt_hyde.QDRANT_COLLECTION_NAME = cfg.collection_name
        use_hyde   = True
        rag_module = None
    else:
        import app.rag.query_transform_v5 as qt_v5
    ...
    app_       = create_app(chroma_db_path=chroma_db_path, use_genre_filter=cfg.use_genre_filter,
                            use_hyde=use_hyde, rag_module=rag_module)

    results = []
    with get_openai_callback() as cb:
        asyncio.run(run_session(app_, persona, results, thread_id, session_id=session_id))
...
if __name__ == "__main__":
    if "--sweep" in sys.argv:
        sweep_id = wandb.sweep(SWEEP_CONFIG, project="peekabook-crs-test7-2")
