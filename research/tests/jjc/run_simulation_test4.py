"""
CRS × UserSim 오케스트레이션 스크립트 (test4).

test3 대비 변경:
- query_transform_v5 사용: 쿼리당 limit 10, 리랭킹 후 10권 반환
- PeekaJudge가 10권 전체 평가 (binary score per rank)
- wandb 로그: book_1~10_score, persona_name, session_id, estimated_cost_usd 추가
- wandb Table: persona_name, session_id 컬럼 추가
- 비용 추정: LangChain 호출은 get_openai_callback, Judge/UserSim은 tiktoken 기반

실행:
    cd /home/jjeong3150/work/peekabook/backend
    python ../research/tests/jjc/run_simulation_test4.py

Sweep 실행:
    cd /home/jjeong3150/work/peekabook/backend
    python ../research/tests/jjc/run_simulation_test4.py --sweep
"""
import asyncio
import copy
import gc
