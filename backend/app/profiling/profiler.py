...
REFLECTION_PROMPT = """\
아래의 현재 세션 정보와 연결된 이전 메모리들을 분석하여, 사용자 프로필의 5가지 항목(독서 목적, 선호 장르, 독서 스타일, 난이도, 현재 맥락)에 대해 **프로필 텍스트만으로는 드러나지 않는 구체적인 선호 방향**을 도출하세요.
...
- 이전 독서 경험에서 만족/불만족했던 요소는 무엇인가?
- 현재 프로필에는 드러나지 않지만, 이전 메모리를 통해 추론 가능한 숨겨진 선호는 무엇인가?

[2단계: 인사이트 도출]
1단계의 분석 결과를 바탕으로, 프로필 5축 각각에 대해 프로필 텍스트를 넘어서는 구체화된 선호를 도출하세요:
1. 독서 목적 구체화: 프로필의 reading_goal이 실제로 의미하는 더 구체적인 독서 경험은?
2. 장르 내 세부 방향: preferred_genre 안에서 특히 끌릴 세부 영역은?
3. 스타일 구체화: reading_style이 실제로 원하는 책의 구성 방식은?
4. 난이도 맥락화: difficulty_level이 현재 상황에서 구체적으로 어떤 수준을 의미하는지?
5. 상황 연결: current_context에서 사용자가 실제로 책에서 기대하는 도움은?

작성 기준:
- 프로필 슬롯의 값을 그대로 반복하지 마세요. 프로필 너머의 구체화만 작성하세요.
- 프로필, 요약, 독서 경험, 연결된 메모리에 명시된 정보만을 근거로 도출하세요.
- 제공된 정보에서 근거를 찾을 수 없는 항목은 도출하지 마세요. 5가지 모두 채울 필요는 없습니다.
- 2단계 인사이트에 "이전에는 ~했지만" 등 대비 표현은 포함하지 마세요. 현재 선호 방향만 서술하세요.
- 연결된 이전 메모리가 없는 경우, 현재 세션의 독서 경험과 대화 맥락에서 추론하세요.
