"""
graph_test1 팩토리 버전 (v3).

test1 대비 변경:
- use_genre_filter 파라미터로 장르 필터 노드 on/off 제어
- rag_module 파라미터로 RAG 구현체 교체 가능 (기본값: query_transform_v4)
"""
import os
...
def create_app(chroma_db_path: str,
    ...
    if use_genre_filter:
        graph.add_node("extract_genre", rag_module.extract_genre_node)
