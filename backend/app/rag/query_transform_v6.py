...
v5 대비 변경:
- extract_genre_node_v2 도입: 대분류 2개, 중분류 3개를 "반드시" 선택하도록 강제
  (v5의 "최대 N개" → "반드시 N개"로 프롬프트 변경)
  장르 필터가 항상 중분류 레벨에서 확정되므로 fallback 분기 발생 빈도 감소
"""
...
사용자 프로파일을 보고 아래 대분류 목록에서 가장 적합한 것을 반드시 3개 선택하세요.
목록에 없는 값은 절대 반환하지 마세요.

대분류 목록: {large_list}
사용자 프로파일: {summary}

JSON으로만 반환: {{"categories": ["소설/시/희곡", "경제경영", "자기계발"]}}
""")

medium_genre_prompt_v2 = ChatPromptTemplate.from_template("""
사용자 프로파일을 보고 아래 중분류 목록에서 가장 적합한 것을 반드시 5개 선택하세요.
목록에 없는 값은 절대 반환하지 마세요.

중분류 목록: {medium_list}
사용자 프로파일: {summary}

JSON으로만 반환: {{"categories": ["한국소설", "외국소설", "경제일반", "자기계발", "한국에세이"]}}
""")
